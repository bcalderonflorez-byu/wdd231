...
function displayCourses(courseList) {
    ...
    courseList.forEach(course => {
        ...
        div.innerHTML = `<h2>${course.subject} ${course.number}: ${course.title}</h2>`;
    ...
    const totalCredits1 = courseList.reduce((sum, course) => sum + course.credits, 0);
    const completedCredits = courseList
        .filter(course => course.completed)
        .reduce((sum, course) => sum + course.credits, 0);
    totalCredits.textContent = `Credits Completed: ${completedCredits} / ${totalCredits1}`;
}
